...
<div class="max-w-4xl mx-auto space-y-6">
  <div class="flex flex-wrap items-center gap-4">
    ...
    <div class="flex-1 min-w-[200px]">
      <h1 class="text-2xl font-bold text-slate-900">Kategori Barang</h1>
        ...
        {#each categories as cat}
          <tr class="hover:bg-slate-50 transition-colors">
            ...
            <td class="p-4 text-right">
              <button class="text-blue-600 hover:text-blue-800 text-sm font-medium" onclick={() => openEdit(cat)}>Ubah</button>
            </td>
          </tr>
        {:else}
          <tr>
...
{#snippet marginFields(f: CategoryForm, idPrefix: string)}
  <div class="grid grid-cols-2 gap-4">
    ...
    <div>
      ...
      <input id="{idPrefix}-target" type="number" min="0" step="0.01" bind:value={f.targetMargin} class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="mis. 30">
    </div>
  </div>
  ...
  <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
      <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
        <h3 class="font-semibold text-lg text-slate-900">Tambah Kategori</h3>
      ...
      <form onsubmit={(e) => { e.preventDefault(); addCategory(); }} class="p-6 space-y-4">
        ...
        <div>
          ...
          <input id="name" type="text" bind:value={form.name} required class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="mis. Screen">
        </div>
...
{#if editingId}
  <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
      <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
        <h3 class="font-semibold text-lg text-slate-900">Ubah Kategori</h3>
