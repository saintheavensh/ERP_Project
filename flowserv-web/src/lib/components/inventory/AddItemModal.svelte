  ...
  <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
      <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
        <h3 class="font-semibold text-lg text-slate-900">Tambah Barang</h3>
      ...
      <form onsubmit={(e) => { e.preventDefault(); inv.addItem(); }} class="p-6 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input id="sku" type="text" bind:value={inv.form.sku} required class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none uppercase" placeholder="mis. SKU-001">
          </div>
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1" for="universalCode">Universal Code *</label>
            <div class="flex gap-2">
              <input id="universalCode" type="text" bind:value={inv.form.universalCode} required class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none uppercase" placeholder="mis. BLP673">
        ...
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1" for="name">Nama Barang (Model) *</label>
          <input id="name" type="text" bind:value={inv.form.name} required class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="mis. Baterai Oppo A3s">
        </div>
        ...
        <div class="pt-4 flex justify-end gap-3">
          <button type="button" class="px-4 py-2 text-slate-600 font-medium hover:bg-slate-50 rounded-lg transition-colors" onclick={() => { inv.showAddModal = false; inv.resetForm(); }}>Batal</button>
