          ...
          <td class="p-4">
            <div class="flex flex-col gap-1 items-start">
              {#if !item.isStockInitialized}
                <span class="inline-flex items-center px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider bg-purple-100 text-purple-700 border border-purple-200">Baru / Belum diatur</span>
              {/if}
              ...
              {:else}
                <span class="inline-flex items-center px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-500 border border-slate-200">Tanpa kecocokan</span>
              {/if}
            </div>
          </td>
          <td class="p-4 text-right">
            <div class="flex items-center justify-end gap-3">
              <a href="/inventory/{item.id}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">Lihat</a>
              <button type="button" class="text-red-500 hover:text-red-700 text-sm font-medium" onclick={() => inv.deleteItem(item.id)}>Hapus</button>
            </div>
          </td>
        </tr>
      {:else}
