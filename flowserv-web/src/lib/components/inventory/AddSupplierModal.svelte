  ...
  <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
      <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
        <h3 class="font-semibold text-lg text-slate-900">Tambah Supplier</h3>
      ...
      <form onsubmit={(e) => { e.preventDefault(); state.addSupplier(); }} class="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="col-span-2">
            ...
            <input id="name" type="text" bind:value={state.form.name} required class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="mis. Bintang Sparepart">
          </div>
        ...
        <div class="pt-4 flex justify-end gap-3">
          <button type="button" class="px-4 py-2 text-slate-600 font-medium hover:bg-slate-50 rounded-lg transition-colors" onclick={() => { state.showAddModal = false; state.resetForm(); }}>Batal</button>
