<script lang="ts">
  ...
  let simData = $derived.by(() => {
    if (!simBrandId || !item.stockBatches) return null;
  ...
  async function handleApply() {
    savingBrand = true;
    await onApplyPrice(simBrandId, simSellingPrice);
    savingBrand = false;
  }
</script>
  ...
  <div class="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col">
      ...
      <div class="flex items-center justify-between p-5 border-b border-slate-100">
        ...
        <button onclick={onClose} aria-label="Tutup Simulasi" class="text-slate-400 hover:text-slate-600 p-2">
      ...
      <div class="p-5 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
        <button onclick={onClose} class="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-800 font-medium rounded-lg transition-colors">
          Batal / Tutup
        </button>
        <button onclick={handleApply} disabled={savingBrand} class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors flex items-center gap-2">
          {#if savingBrand}
