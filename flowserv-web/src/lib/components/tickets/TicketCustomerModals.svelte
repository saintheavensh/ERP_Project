  ...
  <div class="fixed inset-0 bg-slate-900/50 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
      ...
      <div class="mt-6 flex justify-end gap-3">
        <button onclick={() => state.showEditForm = false} class="px-4 py-2 text-slate-600 bg-slate-100 rounded-lg">Cancel</button>
        <button onclick={() => state.saveCustomer()} disabled={state.loading || !state.editCustomerData.name} class="px-4 py-2 bg-blue-600 text-white rounded-lg disabled:opacity-50">Save Changes</button>
      </div>
    </div>
  </div>
{/if}
