...
async function runSeed(): Promise<void> {
  ...
  // inventory -> customers -> transactions -> printer -> payment-methods), and
  // a failure partway through should leave nothing behind rather than a
  // half-seeded database. Payment methods only depend on tenants (from core),
  // so their order among the tail modules is not load-bearing.
