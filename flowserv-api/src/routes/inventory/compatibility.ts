...
router.put('/:id/compatibility', requirePermission('inventory.manage_items'), zValidator('json', updateCompatibilitySchema), auditMiddleware({ action: 'inventory_item.update_compatibility', entityType: 'inventory_item', entityIdParam: 'id', bodyFields: ['resolvedModelIds', 'unresolvedCompatibility'] }), async (c) => {
