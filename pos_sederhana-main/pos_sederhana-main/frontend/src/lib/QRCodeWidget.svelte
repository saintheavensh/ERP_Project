<script>
    ...
    async function fetchLanUrl() {
        try {
            const response = await fetch(`${config.printBase}/api/server-info`);
    ...
    // Reactive statement: generate QR whenever it opens and canvas is ready
    $: if (isOpen && qrCanvas && accessUrl) {
...
<!-- Floating QR Code Widget (center-left) - Hidden on mobile, shown on Desktop -->
<div class="no-print hidden md:block fixed top-1/2 -translate-y-1/2 left-4 z-60" style="top: 50%; transform: translateY(-50%);">
